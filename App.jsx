...
const DEFAULT_DASHBOARD_WIDGETS = [
  ...
  { id: 'selectedDay', label: 'Daily Study Tracker', enabled: true },
...
function baseTasksForLecture(lecture) {
...
function normalizeCustomTasks(customTasks = []) {
  return (Array.isArray(customTasks) ? customTasks : [])
    .map((task, index) => ({
      key: task.key || `custom_${Date.now()}_${index}`,
      label: String(task.label || '').trim(),
      group: task.group || 'Core',
      custom: true
    }))
...
function tasksForLecture(lecture = {}) {
  const hidden = new Set(Array.isArray(lecture.hiddenTaskKeys) ? lecture.hiddenTaskKeys : []);
  return [...baseTasksForLecture(lecture), ...normalizeCustomTasks(lecture.customTasks)]
    .filter((task) => !hidden.has(task.key));
}

function editableTasksForLecture(lecture = {}) {
  const hidden = new Set(Array.isArray(lecture.hiddenTaskKeys) ? lecture.hiddenTaskKeys : []);
  return [...baseTasksForLecture(lecture), ...normalizeCustomTasks(lecture.customTasks)]
    .map((task) => ({ ...task, hidden: hidden.has(task.key) }));
}

function scheduledDateForTask(lecture = {}, task = {}) {
  return lecture.taskDates?.[task.key] || lecture.date || todayISO();
}

function scheduledTaskRowsForDate(lectures = [], date = todayISO()) {
  return lectures
    .map((lecture) => ({
      lecture,
      tasks: tasksForLecture(lecture).filter((task) => scheduledDateForTask(lecture, task) === date)
    }))
    .filter((row) => row.tasks.length > 0);
}

const MEDTRACKER_DRAG_TYPE = 'application/x-medtracker-item';

function setMedTrackerDragData(e, payload) {
  if (!e?.dataTransfer || !payload) return;
  const serialized = JSON.stringify(payload);
  e.dataTransfer.setData(MEDTRACKER_DRAG_TYPE, serialized);
  e.dataTransfer.setData('text/plain', serialized);
  e.dataTransfer.effectAllowed = 'move';
}

function getMedTrackerDragData(e) {
  if (!e?.dataTransfer) return null;
  const raw = e.dataTransfer.getData(MEDTRACKER_DRAG_TYPE) || e.dataTransfer.getData('text/plain');
  if (!raw || raw.startsWith('widget:')) return null;
  try {
    const parsed = JSON.parse(raw);
    if (parsed?.type === 'lecture' && parsed.lectureId) return parsed;
    if (parsed?.type === 'task' && parsed.lectureId && parsed.taskKey) return parsed;
  } catch {
    // Older saved builds dragged lecture ids as plain text. Keep that fallback working.
  }
  return raw ? { type: 'lecture', lectureId: raw } : null;
}
...
function coreTaskCountsForScheduledRows(rows = [], progress = {}) {
  let done = 0;
  let total = 0;
  rows.forEach(({ lecture, tasks }) => {
    const coreTasks = (tasks || []).filter((task) => task.group === 'Core');
    total += coreTasks.length;
    done += coreTasks.filter((task) => taskCompleted(progress[lecture.id], task)).length;
  });
  return { done, total, percent: total ? Math.round((done / total) * 100) : 0 };
}

function coreCompletionPercent(lecture, progress, tasks = tasksForLecture(lecture)) {
  const coreTasks = tasks.filter((task) => task.group === 'Core');
  if (coreTasks.length === 0) return 0;
  const done = coreTasks.filter((task) => taskCompleted(progress, task)).length;
...
function LectureCard({ lecture, progress, visibleTasks, coreScoreOnly = false, onToggle, onDelete, onSave, onMoveToDate, onTaskDateChange, onHideTask, onAddCustomTask, onDragStart, onTaskDragStart }) {
  const tasks = visibleTasks?.length ? visibleTasks : tasksForLecture(lecture);
  const percent = coreScoreOnly ? coreCompletionPercent(lecture, progress, tasks) : completionPercent(lecture, progress);
  const hasCoreTasks = tasks.some((task) => task.group === 'Core');
  const editableTasks = editableTasksForLecture(lecture);
  const [editing, setEditing] = useState(false);
  const [saving, setSaving] = useState(false);
  const [editError, setEditError] = useState('');
  const [newTaskLabel, setNewTaskLabel] = useState('');
  const [newTaskGroup, setNewTaskGroup] = useState('Core');
  ...
  const hiddenCount = editableTasks.filter((task) => task.hidden).length;

  async function handleAddCustomTask() {
    setEditError('');
    if (!newTaskLabel.trim()) {
      setEditError('Enter a checklist item label first.');
      return;
    }
    try {
      await onAddCustomTask?.(lecture.id, {
        label: newTaskLabel.trim(),
        group: newTaskGroup
      });
      setNewTaskLabel('');
      setNewTaskGroup('Core');
    } catch (err) {
      setEditError(err.message || 'Could not add checklist item.');
    }
  }
      ...
      {!editing ? (
        <>
          <div className="lecture-topline">
            ...
            <div className="percent-pill" title={coreScoreOnly ? 'Core checklist only' : 'All checklist items'}>{coreScoreOnly && !hasCoreTasks ? 'Core N/A' : `${percent}%`}</div>
          </div>

          <ProgressBar value={percent} />
          {coreScoreOnly && <p className="muted small-note">Daily Study Tracker score counts Core checklist items only. Board checklist items stay visible but do not change this percent.</p>}
            ...
            return (
              <div key={group} className="task-section">
                ...
                <div className="task-grid">
                  {groupTasks.map((task) => (
                    <div className={taskCompleted(progress, task) ? 'check-chip checked' : 'check-chip'} key={task.key}>
                      <button
                        type="button"
                        className="task-drag-handle"
                        draggable
                        title={`Drag this task to another date: ${lecture.title} — ${task.label}`}
                        onDragStart={(e) => {
                          e.stopPropagation();
                          onTaskDragStart?.(e, lecture.id, task.key);
                        }}
                      >
                        ⇅
                      </button>
                      <label className="check-chip-main">
                      ...
                      <input
                        ...
                        type="date"
                        value={scheduledDateForTask(lecture, task)}
                        aria-label={`Move ${task.label} to another day`}
                        onChange={(e) => onTaskDateChange?.(lecture.id, task.key, e.target.value)}
                      />
                    </div>
                  ))}
                </div>
        ...
        <form className="edit-lecture-form" onSubmit={handleSave}>
          <div className="lecture-topline">
            ...
            <div className="percent-pill">{coreScoreOnly && !hasCoreTasks ? 'Core N/A' : `${percent}%`}</div>
          </div>
          <label>
            Date
          ...
          <div className="checklist-editor">
            <div>
              <p className="eyebrow">Checklist editor</p>
              <h4>Add, schedule, or hide checklist items</h4>
              <p className="muted">Hidden items stop counting toward completion. Custom Core items count in the Daily Study Tracker core score.</p>
            </div>
            <div className="checklist-editor-list">
              {editableTasks.map((task) => (
                <div className={task.hidden ? 'checklist-editor-row hidden' : 'checklist-editor-row'} key={task.key}>
                  <button
                    type="button"
                    className="task-drag-handle editor-handle"
                    draggable
                    title={`Drag this task to another date: ${lecture.title} — ${task.label}`}
                    onDragStart={(e) => {
                      e.stopPropagation();
                  ...
                  <div>
                    <strong>{task.label}</strong>
                    <small>{task.group} checklist{task.custom ? ' · custom' : ' · built in'}</small>
                  </div>
                  <input
                    type="date"
                    value={scheduledDateForTask(lecture, task)}
                    onChange={(e) => onTaskDateChange?.(lecture.id, task.key, e.target.value)}
                  />
                  <button type="button" className="ghost-btn small-btn" onClick={() => onHideTask?.(lecture.id, task.key, !task.hidden)}>
                    {task.hidden ? 'Restore' : task.custom ? 'Delete' : 'Hide'}
                  </button>
                </div>
              ))}
            </div>
            {hiddenCount > 0 && <p className="muted">{hiddenCount} hidden item{hiddenCount === 1 ? '' : 's'} can be restored from this editor.</p>}
            <div className="add-task-row">
              <input value={newTaskLabel} onChange={(e) => setNewTaskLabel(e.target.value)} placeholder="Add a custom checklist item" />
              <select value={newTaskGroup} onChange={(e) => setNewTaskGroup(e.target.value)}>
                {groups.map((group) => <option key={group} value={group}>{group}</option>)}
              </select>
              <button type="button" className="ghost-btn" onClick={handleAddCustomTask}>Add item</button>
            </div>
          </div>
...
function DailyStudyDropZones({ selectedDate, lectures = [], progress = {}, onSelectDate, onDropItem }) {
  const dates = weekDates(selectedDate || todayISO());

  return (
    <div className="daily-study-drop-panel" aria-label="Move lectures or checklist items to another date">
      <div className="daily-study-drop-heading">
        <div>
          <p className="eyebrow">Drag schedule</p>
          <h3>Drop a checklist item onto the day you want to work on it</h3>
          <p className="muted">Use the ⇅ handle on an item, or drag the full lecture card. The task stays linked to its original lecture/lab title.</p>
        </div>
      </div>
      <div className="daily-study-drop-grid">
        {dates.map((date) => {
          const rows = scheduledTaskRowsForDate(lectures, date);
          const taskTotal = rows.reduce((sum, row) => sum + row.tasks.length, 0);
          const coreCounts = coreTaskCountsForScheduledRows(rows, progress);
          return (
            <div
              key={date}
              className={date === selectedDate ? 'daily-drop-date active' : 'daily-drop-date'}
              onDragOver={(e) => {
                e.preventDefault();
                e.dataTransfer.dropEffect = 'move';
              }}
              onDrop={(e) => onDropItem?.(e, date)}
            >
              <button type="button" onClick={() => onSelectDate?.(date)}>
                <strong>{niceDate(date, true)}</strong>
                <span>{taskTotal} item{taskTotal === 1 ? '' : 's'}</span>
                <small>{coreCounts.total ? `${coreCounts.percent}% core` : 'No core items'}</small>
              </button>
              <em>Drop here</em>
            </div>
          );
    ...
    <section className="panel">
      <div className="panel-heading">
        <div>
          ...
          <h2>Drag a lecture or checklist item onto another day to reschedule</h2>
        </div>
      </div>
...
function StudyTimeLogger({ selectedDate, lectures = [], studySessions, onSave, onDelete }) {
  ...
  const [exactStudyMinutes, setExactStudyMinutes] = useState('');
  ...
  function applyExactTime() {
    const totalSeconds = numberOrZero(exactStudyMinutes) * 60;
    if (totalSeconds <= 0) {
      setMessage('Enter the exact number of minutes studied before applying exact time.');
      return;
    }
    setRunning(false);
      ...
      <form className="study-log-form" onSubmit={saveSession}>
        ...
        <div className="study-timer-card">
          ...
          <div className="button-row compact-buttons">
            ...
            <button type="button" onClick={() => { setRunning(false); setSeconds(0); setExactStudyMinutes(''); }}>Reset</button>
          </div>
          <div className="exact-time-row single-exact-field">
            <label>
              Exact minutes studied
              <input type="number" min="0" step="1" value={exactStudyMinutes} onChange={(e) => setExactStudyMinutes(e.target.value)} placeholder="Example: 82" />
            </label>
            <button type="button" className="ghost-btn" onClick={applyExactTime}>Use exact time</button>
          </div>
          <div className="button-row compact-buttons">
            <button type="button" onClick={() => addManualMinutes(5)}>+5 min</button>
            <button type="button" onClick={() => addManualMinutes(10)}>+10 min</button>
            <button type="button" onClick={() => addManualMinutes(15)}>+15 min</button>
            <button type="button" onClick={() => addManualMinutes(30)}>+30 min</button>
            <button type="button" onClick={() => addManualMinutes(45)}>+45 min</button>
...
function FeatureHome({ selectedDate, lectures, progress, dayLectures, overdueLectures, overall, dayScore, dayCoreTotal, recommendation, qbankDays, studySessions, boardChecklist, todos, widgets, onOpen, onReorderWidget }) {
  ...
  const metrics = {
    ...
    selectedDay: {
      ...
      metric: `${dayLectures.length} scheduled`,
      detail: dayCoreTotal ? `${dayScore}% core complete` : 'No core items scheduled'
    },
    allLectures: {
    ...
    <section className="panel homepage-panel">
      ...
      <div className="home-priority-grid">
        <div className="home-priority-card">
          <span>Daily Study Tracker date</span>
          <strong>{niceDate(selectedDate, true)}</strong>
          <small>{dayLectures.length} lecture checklist{dayLectures.length === 1 ? '' : 's'} · {dayCoreTotal ? `${dayScore}% core complete` : 'No core items scheduled'}</small>
      ...
      <div className="home-section-label"><span>Open a feature</span><small>Drag feature cards to customize the homepage order; click a card to open it.</small></div>
      ...
      <div className="home-feature-grid">
        {visibleWidgets.map((widget) => {
          ...
          return (
            <button
              type="button"
              className={draggingWidgetId === widget.id ? 'home-feature-card dragging' : 'home-feature-card'}
              key={widget.id}
              draggable
              onDragStart={(e) => {
              ...
              onDrop={(e) => {
                e.preventDefault();
                const sourceId = e.dataTransfer.getData('text/plain').replace(/^widget:/, '');
                setDraggingWidgetId('');
                if (sourceId) onReorderWidget?.(sourceId, widget.id);
              }}
...
function Dashboard({ user, workspace }) {
  ...
  const today = todayISO();
  const selectedDayTaskRows = scheduledTaskRowsForDate(lectures, selectedDate);
  const dayLectures = selectedDayTaskRows.map((row) => row.lecture);
  ...
  const selectedDayCoreCounts = coreTaskCountsForScheduledRows(selectedDayTaskRows, progress);
  const dayScore = selectedDayCoreCounts.percent;
  ...
  async function moveTaskToDate(lectureId, taskKey, date) {
    const lecture = lectures.find((item) => item.id === lectureId);
    if (!lecture || !taskKey || !date) return;
  ...
  async function hideLectureTask(lectureId, taskKey, shouldHide = true) {
    const lecture = lectures.find((item) => item.id === lectureId);
    if (!lecture || !taskKey) return;
    const customTasks = normalizeCustomTasks(lecture.customTasks);
    const taskDates = { ...(lecture.taskDates || {}) };

    if (customTasks.some((task) => task.key === taskKey) && shouldHide) {
      delete taskDates[taskKey];
      await updateLecture(lectureId, {
        customTasks: customTasks.filter((task) => task.key !== taskKey),
        hiddenTaskKeys: (lecture.hiddenTaskKeys || []).filter((key) => key !== taskKey),
        taskDates
      });
      return;
    }
    ...
    const hidden = new Set(Array.isArray(lecture.hiddenTaskKeys) ? lecture.hiddenTaskKeys : []);
    if (shouldHide) hidden.add(taskKey);
    else hidden.delete(taskKey);
    await updateLecture(lectureId, { hiddenTaskKeys: Array.from(hidden) });
  }

  async function addCustomLectureTask(lectureId, task) {
    const lecture = lectures.find((item) => item.id === lectureId);
    if (!lecture || !task?.label) return;
    const key = `custom_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const nextTask = { key, label: task.label, group: task.group || 'Core', custom: true };
    await updateLecture(lectureId, {
      customTasks: [...normalizeCustomTasks(lecture.customTasks), nextTask],
      taskDates: {
        ...(lecture.taskDates || {}),
        [key]: selectedDate
      }
  ...
  function handleDragStart(e, lectureId) {
    setMedTrackerDragData(e, { type: 'lecture', lectureId });
  }

  function handleTaskDragStart(e, lectureId, taskKey) {
    setMedTrackerDragData(e, { type: 'task', lectureId, taskKey });
  }

  async function handleDropLecture(e, date) {
    e.preventDefault();
    const payload = getMedTrackerDragData(e);
    if (!payload || !date) return;
    if (payload.type === 'task') {
      await moveTaskToDate(payload.lectureId, payload.taskKey, date);
      setSelectedDate(date);
      return;
    }
    if (payload.type === 'lecture') {
      await moveLectureToDate(payload.lectureId, date);
    }
  }
  ...
  function renderSelectedDay() {
    return (
      <section className="panel">
        <div className="panel-heading">
          <div>
            <p className="eyebrow">Daily Study Tracker</p>
            <h2>{selectedDayTaskRows.length ? `${selectedDayTaskRows.length} lecture/lab checklist${selectedDayTaskRows.length === 1 ? '' : 's'}` : 'No checklist items scheduled for this day'}</h2>
            <p className="muted">This score counts Core checklist items only. Add custom tasks, delete or hide items you do not need, or drag individual checklist items to the day you want to complete them.</p>
          </div>
          <div className="score-badge">{selectedDayCoreCounts.total ? `${dayScore}% core complete` : 'No core items'}</div>
        </div>
        <DailyStudyDropZones
          ...
          onDropItem={handleDropLecture}
        />
        <div className="lecture-list">
          {selectedDayTaskRows.map(({ lecture, tasks }) => (
            <LectureCard
              key={lecture.id}
              lecture={lecture}
              ...
              onTaskDateChange={moveTaskToDate}
              onHideTask={hideLectureTask}
              onAddCustomTask={addCustomLectureTask}
    ...
    if (activeFeature === 'home') {
      return (
        <FeatureHome
          ...
          overall={overall}
          dayScore={dayScore}
          dayCoreTotal={selectedDayCoreCounts.total}
          ...
          onReorderWidget={reorderDashboardWidget}
        />
      );
    }
  ...
  return (
    <main className="dashboard simplified-dashboard">
      ...
      <section className="stats-grid productivity-stats simple-stats">
        ...
        <div className="stat-card"><span>Daily tasks</span><strong>{dayLectures.length}</strong></div>
